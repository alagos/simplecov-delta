...
A **Docker-based GitHub Action** that merges SimpleCov coverage results from parallel test runs, compares them against a stored baseline, and reports per-file coverage deltas via:
...
- **Merge parallel coverage** — Collate `.resultset.json` files from N parallel test jobs using `SimpleCov.collate`
- **Baseline comparison** — Compute per-file and per-group coverage deltas against a cached baseline
- **Inline annotations** — Uncovered lines appear directly in PR diff view
- **Branch-aware caching** — Automatic baseline lookup: current branch → PR target → default branch
- **Zero config for SimpleCov users** — Works with any Ruby project already using SimpleCov
...
The restore-keys cascade in the example above implements this hierarchy automatically.
...
2. **Upload `.resultset.json` as artifacts** — Each parallel job uploads its result
